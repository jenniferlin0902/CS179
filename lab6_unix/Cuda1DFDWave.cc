...
int main(int argc, char* argv[]) {
  ...
  /************************* GPU Implementation *****************************/
   ...
   // cudaMalloc(&dev_data, sizeof(float*)*3);
   // checkCUDAKernelError();
   // checkCUDAKernelError();
    for (int i = 0; i<3; i++){
        cudaMalloc(&(dev_data[i]), sizeof(float) * numberOfNodes);
        checkCUDAKernelError();
        cudaMemset(dev_data[i], 0x0, sizeof(float) * numberOfNodes);
        checkCUDAKernelError();
    }
    ...
    for (size_t timestepIndex = 0; timestepIndex < numberOfTimesteps;
        ...
        // nickname displacements
        float *dev_oldDisplacements = dev_data[(timestepIndex - 1) % 3];
        float *dev_currentDisplacements = dev_data[(timestepIndex + 0) % 3];
        float *dev_newDisplacements = dev_data[(timestepIndex + 1) % 3];

        /* TODO: Call a kernel to solve the problem (you'll need to make
        the kernel in the .cu file) */
        callCuda1DFDWaveKernal(dev_newDisplacements, dev_currentDisplacements, dev_oldDisplacements,
                               courantSquared, numberOfNodes, threadsPerBlock, blocks);
        //checkCUDAKernelError();
        ...
        callCuda1DWaveBCKernal(dev_newDisplacements, left_boundary_value, numberOfNodes, threadsPerBlock, blocks);
       // checkCUDAKernelError();
        ...
        if (CUDATEST_WRITE_ENABLED == true && numberOfOutputFiles > 0 &&
            ...
            cudaMemcpy(file_output, dev_newDisplacements, sizeof(float) * numberOfNodes, cudaMemcpyDeviceToHost);
            checkCUDAKernelError();
